...
const emptyForm = { tag: "", title: "", description: "", metrics: "", imageUrl: "" };
...
export default function AdminCaseStudies() {
  ...
  const openEdit = (cs: CaseStudy) => {
    setEditing(cs);
    setForm({ tag: cs.tag, title: cs.title, description: cs.description, metrics: cs.metrics, imageUrl: cs.imageUrl });
    setDialogOpen(true);
  };
          ...
          {items.map((cs) => (
            <div key={cs.id} className="bg-card border border-border rounded-xl p-5 flex gap-4 hover:shadow-card transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-semibold text-foreground mt-1">{cs.title}</h3>
                <p className="text-sm text-muted-foreground mt-0.5 line-clamp-2">{cs.description}</p>
              </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Title *</Label>
              <Input placeholder="e.g. Scaled Engineering Team by 3x in 60 Days" value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <Label>Description</Label>
              <Textarea rows={3} placeholder="What was the challenge and outcome?" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <Label>Metrics</Label>
              <Input placeholder="e.g. 15 Engineers • 40% Faster Hiring • 2 Months" value={form.metrics} onChange={(e) => setForm((p) => ({ ...p, metrics: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
